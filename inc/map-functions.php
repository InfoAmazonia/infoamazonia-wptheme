...
function mappress_scripts() {
	...
	wp_enqueue_script('mappress', get_template_directory_uri() . '/js/mappress.js', array('mapbox-js', 'underscore', 'jquery'), '0.0.6.42');
	...
	wp_enqueue_script('mappress.groups', get_template_directory_uri() . '/js/mappress.groups.js', array('mappress', 'underscore'), '0.0.3.20');
	wp_enqueue_script('mappress.markers', get_template_directory_uri() . '/js/mappress.markers.js', array('mappress', 'underscore'), '0.0.2.28');
...
function mappress_get_map_data($map_id = false) {
	...
	$data['legend_page'] = get_post_meta($map_id, 'legend_page', true);
	return $data;
}

add_action('wp_ajax_nopriv_map_details', 'mappress_get_map_details');
add_action('wp_ajax_map_details', 'mappress_get_map_details');
function mappress_get_map_details() {
	global $post;
	$post = get_post(intval($_REQUEST['page_id']));
	if($post) {
		setup_postdata($post);
		$content = '<h2>' . get_the_title() . '</h2>';
		$content .= '<div class="details-content">' . apply_filters('the_content', get_the_content()) . '</div>';
		echo $content;
		wp_reset_postdata();
	}
	exit;
